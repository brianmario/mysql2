...
ActiveRecord driver: it is possible to set any appropriate (with respect to the tz[http://www.twinsun.com/tz/tz-link.htm] database) time zone in database.yml with "time_zone" configuration option.
This has the same effect as setting the "@@session.time_zone" MySQL variable at the beginning of the session.
